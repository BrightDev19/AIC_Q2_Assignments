...
def function10():
    ...
    brr = np.array([1, 2, 3])
    subt = np.vstack((arr[0, :]-brr[0], arr[1, :]-brr[1], arr[2, :]-brr[2]))
...
def function12():
    ...
    arr = np.array([1, 2, 3])
    ans = np.hstack((arr[0], arr[0], arr[0], arr[1], arr[1],
                     arr[1], arr[2], arr[2], arr[2], arr, arr))
...
def function17():
    ...
    test = (arr % 3 == 0) | (arr % 5 == 0)
    return np.where(test, 'Yes', 'No')
